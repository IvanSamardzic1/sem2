...
const MyComponent2 = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Dohvaćanje podataka sa servera "https://api.idissuer.si"
        const response1 = await fetch('api/infostat', {
          headers: {
        ...
        const response2 = await fetch('api/ver', {
          headers: {
            'Content-Type': 'application/json'
          }
        });
...
return (
  <div>
    ...
    <h2>InfoStat https://api.idissuer.si:</h2>
    <pre>{JSON.stringify(infoStat, null, 2)}</pre>
    <h2>Ver https://api.idissuer.si:</h2>
